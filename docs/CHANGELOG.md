...
**字段定义默认值支持**：
- `ParameterSchema` 的 `schema` 字典中的字段定义现在支持 `default` 字段
- 在验证数据时，如果字段不存在且有默认值，会自动填充默认值
- 支持在字段定义中同时使用 `type`、`required`、`description`、`default` 四个字段

### 🔧 功能改进

**字段定义格式增强**：
- 字段定义现在支持完整的元数据：`type`、`required`、`description`、`default`
- 验证逻辑优化：先应用默认值，再检查必传字段
- 保持向后兼容：简单格式（`"field": "string"`）仍然有效
...
# data 现在会是: {"url": "https://api.example.com", "timeout": 30.0, "method": "GET"}
```

### ⚠️ 注意事项

- 默认值会在验证时自动应用到数据字典中
- 如果字段不存在且没有默认值，但 `required=True`，会抛出错误
- 默认值的类型必须与字段定义的类型匹配
